      ...
      >
        <div className="h-full flex flex-col">
          <div className="flex flex-1 justify-between items-center px-3 pb-2 border-b">
            <h1 className="font-bold text-xl">Detail Pembelian</h1>
            <span
                    ...
                    <table className="text-center w-full md:text-lg ">
                      ...
                      <tbody>
                        {item.products.map((i, index) => (
                          <tr className="border-b border-neutral-200" key={i.id}>
                            <td className="whitespace-nowrap px-6 py-1">{index + 1}</td>
                            <td className="whitespace-nowrap px-6 py-1">{i.id}</td>
                            <td className="whitespace-nowrap px-6 py-1">{i.title}</td>
                            <td className="whitespace-nowrap px-6 py-1">
                              {currencyToNumber(i.buyPrice).toLocaleString("id-ID", {
                                style: "currency",
                                currency: "IDR",
                                minimumFractionDigits: 0,
                              })}
                            </td>
                            <td className="whitespace-nowrap px-6 py-1">{i.quantity}</td>
                            <td className="whitespace-nowrap px-6 py-1">
                              {currencyToNumber(i.subTotal.toString()).toLocaleString("id-ID", {
                                style: "currency",
                                currency: "IDR",
                                minimumFractionDigits: 0,
                              })}
                      ...
                      <tfoot>
                        <tr className="sticky bottom-0 bg-blue-100">
                          ...
                          <td className="whitespace-nowrap px-6 py-3 font-bold w-1/6">
                            {item.totalItem}
                          </td>
                          <td className="whitespace-nowrap px-6 py-3 font-bold w-1/6">
                            {currencyToNumber(item.totalPrice.toString()).toLocaleString("id-ID", {
                              style: "currency",
                              currency: "IDR",
                              minimumFractionDigits: 0,
                            })}
